...
##############################
def cal(point, start, end, minPeriod, maxPeriod, filenames, pic,up_percent_0=0.7):
    '''    
    Main function.    
    ...
    point : int
            time poits in 24hr
    start : int
            analytic start point
    end : int
            analytic end point
    minPeriod : int
            minimal period of interest 
    maxPeriod : int
            maximum period of interest
    filenames : str
            raw data file path
    pic : str
            yes or no, whether save picture
    up_percent_0 : float [0,1]
            quality control for 0 value percent
    ...
    for inputPath in filenames:
        ...
        parent_path = os.path.join(parent_path, 'result')
        ...
        def inputFile(inputPath):
            ...
            for index,row in df.iterrows():
                ...
                if percent_0 > up_percent_0:
                    ...
                    print('Data(sample:{0}) quality control success!'.format(index))
...
'''
f = [r'E:\codetest\FFT\0620\RCAT_WIN\test data\s2_1.csv'] # file path
pic = 'no' # do not output figures, set default, change to 'yes' if you want figures
# (allresult, Pathlist) = cal(6, 0, 64, 16, 32, f, pic,up_percent_0=0.7)
re = cal(6, 0, 64, 16, 32, f, pic)
'''
