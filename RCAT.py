...
class MyApp(QtGui.QMainWindow, Ui_MainWindow):
    ...
    def runs(self):
        ...
        if not len(allfilepath):
            ...
            db = int(self.startPoint.text())
            ub = int(self.endPoint.text())
            ...
            if not len(selectedRow):
                ...
                try:
                    (allresult, Pathlist) = fft.cal(points,db,ub,tempfilepath)
                except (KeyError, TypeError, IndexError, BaseException) as error:
                    self.statusBar.showMessage('System has error,please check inputFile.')
                    
                for i in range(len(tempfilepath)):#i is result Matrix
                    index = int(selectedRow[i].row()) #index is row selected
                    Imgpath[index] = Pathlist[i]
                    for j in range(5):
                        self.newItem = QTableWidgetItem(allresult[i+1][j])
                        self.newItem.setTextAlignment(QtCore.Qt.AlignHCenter | QtCore.Qt.AlignVCenter)
                        self.tw.setItem(index,j,self.newItem)
